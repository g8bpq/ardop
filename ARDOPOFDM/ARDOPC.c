...
void SendCalibrate(char * strMycall, char * strTargetCall);
...
BOOL NeedCal = FALSE;		// CALiBRATE Command Flag
BOOL NeedCQ = FALSE;		// CQ Command Flag
...
const char strFrameType[64][18] =
{
	...
	"16QAM.200.100.O",
	"CALREQUEST", "CALREPSONSE",// 0x16 to 0x17
...
int EncodePing(char * strMyCallsign, char * strTargetCallsign, UCHAR * bytReturn)
{
	...
	CompressCallsign(strTargetCallsign, &bytToRS[6]); 

	RSEncode(bytToRS, &bytReturn[14], 12, 4);  // Generate the RS encoding ...now 14 bytes total
	return 18;
}

int EncodeCal(char * strMyCallsign, char * strTargetCallsign, UCHAR * bytReturn)
{
	// Encodes a 4FSK 200 Hz BW Ping frame ( ~ 1950 ms with default leader/trailer) 

	UCHAR * bytToRS= &bytReturn[2];

	bytReturn[0] = CALREQUEST;
	bytReturn[1] = bytReturn[0] ^ 0xFF; // Ping always uses session ID of &HFF
...
void CheckTimers()
{
	...
	{
		NeedCal = 0;
		SendCalibrate(Callsign, ConnectToCall);
	}
...
void SendCalibrate(char * strMycall, char * strTargetCall)
{   	
	EncLen = EncodeCal(strMycall, strTargetCall, bytEncodedBytes);
	...
	WriteDebugLog(LOGDEBUG, "[SendCAL] strMycall= %s strTargetCall=%s", strMycall, strTargetCall);
