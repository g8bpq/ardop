...
int Acquire4FSKFrameType()
{
	...
	if (IsConReqFrame(NewType) || NewType == PING || NewType == CALREQUEST)
...
*/
		...
		{	
			...
			// If Calibrate or Cal Response we need to collect the tone content

			if (intFrameType == CALREQUEST || intFrameType == CALRESPONSE)
			{
				State = GettingTone;
