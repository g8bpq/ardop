...
// This is my port of UZ7HO's soundmodem. 

// Start with basic ax.25 receive

VOID UZ7HOProcessSamples(short * Samples, int nSamples)
{
}
...
const char sErrPrimeTooLarge[] = "Prime factor for FFT length too large. Change value for cMaxPrimeFactor in FFTs unit";
...
void ForwardFFT(const Source: array of TComplex; var Dest: array of TComplex; Count: integer);
...
*/
void InverseFFT(struct _Dcomplex * Source, struct _Dcomplex * Dest, int Count);
...
int TIdx0FactorArray[cMaxFactorCount];
int TIdx1FactorArray[cMaxFactorCount + 1];

// Factorise the series with length Count into FactorCount factors, stored in Fact

void Factorize(int Count, int FactorCount, int * Fact)
{
}
